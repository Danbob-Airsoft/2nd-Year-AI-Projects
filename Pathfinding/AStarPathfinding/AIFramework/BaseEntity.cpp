...
void BaseEntity::GeneratePath(NodeClass* ActiveGrid[40][40])
{
	...
	//Make Frontier
	std::priority_queue<Node_P, std::vector<Node_P>, std::greater<Node_P>> Frontier;
	bool TargetFound = false;

	for (int i = 0; i < 40; i++)
	{
		for (int j = 0; j < 40; j++) {
			ActiveGrid[i][j]->TentativeDistance = ActiveGrid[i][j] == StartNode ? 0 : INT_MAX;
		}
	}
	...
	while (Frontier.size() > 0 && TargetFound == false)
	{
		...
		NodeClass* CurrentNode = Frontier.top().second;
