...
public:
	 ...
	 */
	explicit BaseEntity(NodeClass* ActiveGrid[40][40]);
	/**
	 * Constructor that accepts a texture file name
	 * @param file a string filename
	 */
	explicit BaseEntity(std::string file, NodeClass* ActiveGrid[40][40]);
	/**
	 * Constructor that accepts a texture file name and a colour tint
	 * @param file a string filename
	 * @param colour a sf::Color value
	 */
	explicit BaseEntity(std::string file, sf::Color colour, NodeClass* ActiveGrid[40][40]);
	 ...
	 */
	virtual void Think(NodeClass* ActiveGrid[40][40]);
	...
	*/
	...
	void GeneratePath(NodeClass* ActiveGrid[40][40]);
protected:
	std::string filename;
	sf::Sprite sprite;
	sf::Texture texture;
	...
	//Node to start building path from
	NodeClass* StartNode;

	//Give Entity a path
	std::queue<NodeClass*> Path;

	int CurrentCostOfPath;

	//Locate and look at top node in path
	void FindCurrentTarget(NodeClass* ActiveGrid[40][40]);

	//Move Towards current target and check for arrival
	void ArrivalCheck(NodeClass* CurrentTarget, NodeClass* ActiveGrid[40][40]);

	//Get Input for new Target
	NodeClass* GetRandomNode(NodeClass* ActiveGrid[40][40]);
...
protected:
	 ...
	 */
	virtual void Initialize(NodeClass* ActiveGrid[40][40]);
};
